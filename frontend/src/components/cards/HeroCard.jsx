...
function GlitchBadge({ text, color }) {
  ...
  return (
    ...
    >
      ...
      />
      <span style={{ fontSize: '13px', fontWeight: '700', color, fontFamily: 'monospace', letterSpacing: '0.08em' }}>
        {display}
      </span>
    </motion.div>
  )
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: `1px solid ${C.amberBorder}`, borderRadius: '12px',
        padding: '16px', marginTop: '16px', position: 'relative', overflow: 'hidden',
      }}
    >
      <motion.div
        animate={{ x: ['-100%', '200%'] }}
      ...
      />
      <p style={{
        fontSize: '9px', fontWeight: '600', color: C.amber, letterSpacing: '0.15em',
        textTransform: 'uppercase', marginBottom: '8px',
        display: 'flex', alignItems: 'center', gap: '6px',
      }}>
        <span>◈</span> Hidden advantage
      </p>
      <p style={{ fontSize: '13px', color: C.text, lineHeight: '1.6' }}>{text}</p>
    </motion.div>
  )
}
...
function SectionLabel({ children }) {
  return (
    <p style={{
      ...
      color: C.amber, textTransform: 'uppercase', marginBottom: '12px',
...
function GlassCard({ children, style = {}, delay = 0, glowColor }) {
  ...
  return (
    <motion.div ref={ref} variants={cardMaterialize} initial="hidden" animate={inView ? 'visible' : 'hidden'} transition={{ delay }}>
      <div
        ref={tilt.ref}
...
export default function HeroCard({ data }) {
  ...
  return (
    <div id="act1" style={{ paddingTop: '32px' }}>
      ...
      <div style={{
        ...
        <GlassCard delay={0.1} glowColor={C.scoreColor(score)}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0' }}>
            <ReadinessRing score={score} size={isMobile ? 140 : 180} />
          </div>
        </GlassCard>

        <GlassCard delay={0.2}>
          <SectionLabel>Current level</SectionLabel>
          <GlitchBadge text={level.toUpperCase()} color={C.scoreColor(score)} />
          {summary.overview && (
            <p style={{ fontSize: '13px', color: C.textMuted, lineHeight: '1.7', marginTop: '14px' }}>
              {summary.overview}
            </p>
          )}
      ...
      {/* strengths + gaps — stack on mobile */}
      {((summary.strengths?.length > 0) || (summary.honest_gaps?.length > 0)) && (
        <div style={{
          ...
          {summary.strengths?.length > 0 && (
            <GlassCard delay={0.5} style={{ borderLeft: `2px solid ${C.teal}66` }}>
              <SectionLabel>Strengths</SectionLabel>
              {summary.strengths.map((s, i) => (
                <BulletItem key={i} text={s} color={C.teal} prefix="→" delay={i * 0.08} />
              ))}
            </GlassCard>
          )}
          {summary.honest_gaps?.length > 0 && (
            <GlassCard delay={0.6} style={{ borderLeft: `2px solid ${C.red}66` }}>
              <SectionLabel>Gaps</SectionLabel>
              {summary.honest_gaps.map((g, i) => (
                <BulletItem key={i} text={g} color={C.red} prefix="•" delay={i * 0.08} />
              ))}
            </GlassCard>
          )}
        </div>
