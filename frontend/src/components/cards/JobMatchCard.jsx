...
function CollisionDisplay({ matchPercent, inView, color, isMobile }) {
  const [exploded, setExploded] = useState(false)
  const { display, settled, start } = useSlotMachine(matchPercent, { scrambleDuration: 800, delay: 600 })
    ...
    <div style={{
      ...
      <motion.div
        ...
        style={{
          ...
          // On mobile use shorter label text, no nowrap
          padding: '10px 12px', fontSize: isMobile ? '10px' : '11px',
        ...
        {exploded && [0, 1, 2].map(i => (
          <motion.div
            ...
            style={{
              ...
              borderRadius: '50%', border: `1px solid ${color}`, pointerEvents: 'none',
            }}
          />
        ))}
        <div style={{
          ...
          position: 'relative', boxShadow: `0 0 20px ${color}44`,
        }}>
          <motion.p
            animate={settled ? {
              textShadow: [`0 0 0px ${color}`, `0 0 16px ${color}`, `0 0 0px ${color}`],
            } : {}}
            transition={{ duration: 2, repeat: Infinity }}
            style={{ fontSize: '20px', fontWeight: '700', color, lineHeight: '1', fontFamily: 'monospace' }}
          >
            {display}
          </motion.p>
      ...
      <motion.div
        ...
        transition={{ delay: 0.2, duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
        style={{
          background: C.amberGlowSm, border: `1px solid ${C.amberBorder}`,
...
function MatchBar({ pct, color, inView }) {
  return (
    <div style={{
      width: '100%', height: '6px', background: 'rgba(30,42,58,0.8)',
...
function MatchedItem({ req, index }) {
  ...
  return (
    <motion.div
      ...
      style={{ borderLeft: `2px solid ${C.teal}55`, paddingLeft: '12px', marginBottom: '10px' }}
    >
...
function GapItem({ req, index }) {
  ...
  const importanceColor = {
    'critical': C.red, 'important': C.amber, 'nice to have': C.textMuted,
  ...
  return (
    <motion.div
      ...
      animate={inView ? { opacity: 1, x: 0 } : {}}
      transition={{ delay: index * 0.08, duration: 0.4 }}
      style={{ borderLeft: `2px solid ${C.red}44`, paddingLeft: '12px', marginBottom: '12px' }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
        ...
        {req.importance && (
          <span style={{
            fontSize: '9px', fontWeight: '600', color: importanceColor,
            background: `${importanceColor}15`, border: `1px solid ${importanceColor}33`,
            borderRadius: '4px', padding: '1px 7px', letterSpacing: '0.06em', textTransform: 'uppercase',
          }}>
            {req.importance}
          </span>
        )}
      ...
      {req.time_to_close && (
        <p style={{ fontSize: '10px', color: C.textMuted, marginBottom: '3px' }}>Time to close: {req.time_to_close}</p>
      )}
      {req.how_to_close && (
        <div style={{ display: 'flex', gap: '6px' }}>
          <span style={{ color: C.teal, fontSize: '11px', marginTop: '1px' }}>→</span>
          <p style={{ fontSize: '11px', color: C.teal, lineHeight: '1.5' }}>{req.how_to_close}</p>
        </div>
      )}
    </motion.div>
  )
...
function ApplyBadge({ recommendation }) {
  ...
  }[recommendation] || { text: '◎ See verdict below', color: C.textMuted, bg: 'transparent', border: C.border }
    ...
    <div id="act5-jobmatch" ref={ref}>
      <motion.div
        ...
        style={{
          ...
          borderRadius: '16px', padding: isMobile ? '16px' : '24px', overflow: 'hidden',
        }}
      >
        <SectionLabel>Job match analysis</SectionLabel>

        <CollisionDisplay matchPercent={pct} inView={inView} color={color} isMobile={isMobile} />
        ...
        {jobMatch.summary && (
          <motion.p
            ...
            style={{ fontSize: '13px', color: C.textMuted, lineHeight: '1.6', marginBottom: '12px' }}
          >
            {jobMatch.summary}
          </motion.p>
        )}

        {jobMatch.apply_recommendation && <ApplyBadge recommendation={jobMatch.apply_recommendation} />}

        <MatchBar pct={pct} color={color} inView={inView} />

        {/* matched + gaps — stack on mobile */}
        <div style={{
          display: 'grid',
          ...
          {jobMatch.matched_requirements?.length > 0 && (
            <motion.div
              ...
              style={{
                background: 'rgba(20,184,166,0.04)', border: `1px solid ${C.tealBorder}`,
                borderRadius: '10px', padding: '14px',
              }}
            >
              <p style={{ fontSize: '9px', fontWeight: '600', color: C.teal, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '12px' }}>
                You meet these
              </p>
          ...
          {jobMatch.missing_requirements?.length > 0 && (
            ...
            >
              <p style={{ fontSize: '9px', fontWeight: '600', color: C.red, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '12px' }}>
                Gaps to close
              </p>
        ...
        {jobMatch.verdict && (
          <motion.div
            ...
            style={{
              background: 'rgba(10,15,26,0.6)', border: `1px solid ${C.border}`,
              borderRadius: '10px', padding: '14px',
            }}
          >
            <p style={{ fontSize: '9px', fontWeight: '600', color: C.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '8px' }}>
              Verdict
            </p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <span style={{ color: C.textMuted, fontSize: '11px', marginTop: '2px' }}>◎</span>
              <p style={{ fontSize: '13px', color: C.textMuted, lineHeight: '1.6' }}>{jobMatch.verdict}</p>
            </div>
          </motion.div>
        )}
      </motion.div>
