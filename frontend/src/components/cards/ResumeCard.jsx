// ─────────────────────────────────────────────────────
// ResumeCard.jsx — RESPONSIVE
// Act 3 — The Wound — Resume review
// ─────────────────────────────────────────────────────
...
function SectionLabel({ children, color = C.amber }) {
  return (
    <p style={{
      ...
      <span style={{ display: 'inline-block', width: '14px', height: '1px', background: color }} />
      {children}
    </p>
  )
}
...
function AnimatedDiff({ example, inView }) {
  ...
  const before = parts[0]?.replace('before:', '').trim() || example
  const after  = parts[1]?.trim() || ''

  return (
    <div style={{
      background: 'rgba(8,12,20,0.9)', border: `1px solid ${C.border}`,
      borderRadius: '8px', padding: '12px', fontFamily: 'monospace',
      fontSize: '11px', marginBottom: '8px', overflow: 'hidden',
    }}>
      <AnimatePresence>
        {phase >= 1 && (
          <motion.div
            ...
            style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: after ? '6px' : 0 }}
          >
            <span style={{ color: C.red, fontSize: '10px', marginTop: '1px', flexShrink: 0 }}>−</span>
            <span style={{
              color: C.textMuted,
      ...
      {phase >= 3 && after && (
        ...
        >
          <span style={{ color: C.teal, fontSize: '10px', marginTop: '1px', flexShrink: 0 }}>+</span>
    ...
    >
      <div
        ...
        style={{
          ...
          borderLeft: `2px solid ${C.red}55`, borderRadius: '10px',
          padding: '16px', position: 'relative', overflow: 'hidden',
          height: '100%', transition: css.normal,
        }}
      >
        <motion.div
          initial={{ y: '-105%' }}
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          {spot.section && (
            <span style={{
              ...
              background: 'rgba(30,42,58,0.8)', border: `1px solid ${C.border}`,
          ...
          {spot.issue && (
            <div style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
              ...
              <p style={{ fontSize: '13px', fontWeight: '600', color: C.text, lineHeight: '1.4' }}>{spot.issue}</p>
            </div>
          )}
          {spot.why_it_hurts && (
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              <span style={{ color: '#f97316', fontSize: '11px', marginTop: '2px', flexShrink: 0 }}>•</span>
              <p style={{ fontSize: '12px', color: C.textMuted, lineHeight: '1.5' }}>{spot.why_it_hurts}</p>
            </div>
          )}
          {spot.example && <AnimatedDiff example={spot.example} inView={inView} />}
          {spot.fix && (
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
              ...
              <p style={{ fontSize: '12px', color: C.teal, fontWeight: '500', lineHeight: '1.5' }}>{spot.fix}</p>
            </div>
          )}
        </div>
      </div>
...
export default function ResumeCard({ data }) {
  ...
  return (
    <div id="act3-resume" ref={ref}>
      ...
      <div style={{
        background: C.card, border: `1px solid ${C.redBorder}`,
        borderTop: `2px solid ${C.red}`, borderRadius: '16px', padding: '24px',
      }}>
        ...
        {resumeSummary.overall_impression && (
          ...
          >
            <p style={{ fontSize: '9px', color: C.textMuted, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>
              First impression
            </p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <span style={{ color: C.textMuted, fontSize: '11px', marginTop: '2px' }}>◎</span>
              <p style={{ fontSize: '13px', color: C.textMuted, lineHeight: '1.6' }}>{resumeSummary.overall_impression}</p>
            </div>
            ...
            {(resumeSummary.biggest_strength || resumeSummary.biggest_problem) && (
              <div style={{
                ...
                {resumeSummary.biggest_strength && (
                  <div>
                    <p style={{ fontSize: '9px', color: C.teal, fontWeight: '600', marginBottom: '4px' }}>BEST PART</p>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <span style={{ color: C.teal, fontSize: '11px' }}>→</span>
                      <p style={{ fontSize: '12px', color: C.textMuted, lineHeight: '1.5' }}>{resumeSummary.biggest_strength}</p>
                    </div>
                  </div>
                )}
                {resumeSummary.biggest_problem && (
                  <div>
                    <p style={{ fontSize: '9px', color: C.red, fontWeight: '600', marginBottom: '4px' }}>BIGGEST PROBLEM</p>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <span style={{ color: C.red, fontSize: '11px' }}>•</span>
                      <p style={{ fontSize: '12px', color: C.textMuted, lineHeight: '1.5' }}>{resumeSummary.biggest_problem}</p>
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* issue cards — 2 cols desktop, 1 col mobile */}
        <div style={{
          display: 'grid',
          gridTemplateColumns: isMobile || spots.length === 1 ? '1fr' : '1fr 1fr',
          gap: '10px', marginBottom: '20px', alignItems: 'start',
        }}>
          {spots.map((spot, i) => (
        ...
        {quickWins.length > 0 && (
          <motion.div
            ...
            style={{
              background: 'rgba(20,184,166,0.05)', border: `1px solid ${C.tealBorder}`,
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {quickWins.map((win, i) => <QuickWin key={i} text={win} index={i} />)}
            </div>
          </motion.div>
        )}
      </div>
