...
function FlipNumber({ num, delay = 0 }) {
  const text               = `0${num}`
  const { display, start } = useScrambleText(text, { duration: 500, delay, chars: '0123456789' })
  const { ref, inView }    = useInView(0.3)
  useEffect(() => { if (inView) start() }, [inView])

  return (
    <span ref={ref} style={{ fontFamily: 'monospace', fontSize: '11px', fontWeight: '700', color: C.amber, letterSpacing: '0.1em' }}>
      {display}
    </span>
  )
}
    ...
    >
      <div
        ...
        style={{
          ...
          borderRadius: '12px', padding: '18px', height: '100%',
          transition: css.normal, position: 'relative', overflow: 'hidden',
        }}
      >
        <motion.div
          initial={{ y: '-100%' }}
        ...
        <p style={{ fontSize: '13px', fontWeight: '600', color: C.text, marginBottom: '8px', lineHeight: '1.4' }}>
          {item.insight}
        </p>
        {item.why_it_matters && (
          <p style={{ fontSize: '12px', color: C.textMuted, marginBottom: '10px', lineHeight: '1.6' }}>
            {item.why_it_matters}
          </p>
        )}
        {item.action && (
    ...
    <div id="act2-insights" ref={ref}>
      <motion.div
        ...
        style={{
          ...
          borderTop: `2px solid ${C.amber}`, borderRadius: '16px', padding: '24px',
        }}
      >
        ...
        {meta.headline && (
          ...
          >
            <p style={{ fontSize: '17px', fontWeight: '600', color: C.text, lineHeight: '1.4' }}>
              {meta.headline}
            </p>
          </motion.div>
        )}

        {/* insight grid — 3 cols desktop, 1 col mobile */}
        ...
        <div style={{
          ...
          {meta.realistic_timeline && (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <p style={{ fontSize: '12px', color: C.textMuted, lineHeight: '1.6' }}>{meta.realistic_timeline}</p>
              </div>
            </motion.div>
          )}
          ...
          {meta.biggest_risk && (
            ...
            >
              <p style={{ fontSize: '9px', fontWeight: '600', color: C.red, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '8px' }}>
                Biggest risk
              </p>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                <span style={{ color: C.red, fontSize: '12px' }}>•</span>
                <p style={{ fontSize: '12px', color: C.textMuted, lineHeight: '1.6' }}>{meta.biggest_risk}</p>
              </div>
              {meta.biggest_risk_solution && (
                <div style={{ display: 'flex', gap: '6px' }}>
                  <span style={{ color: C.teal, fontSize: '12px' }}>→</span>
                  <p style={{ fontSize: '12px', color: C.teal, lineHeight: '1.5' }}>{meta.biggest_risk_solution}</p>
                </div>
              )}
            </motion.div>
          )}
