    ...
    <div ref={ref} style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
      {words.map((word, i) => (
        <motion.span
          ...
          style={{ fontSize: '22px', fontWeight: '700', color: C.text, letterSpacing: '-0.01em', lineHeight: '1.2' }}
        >
          {word}
        </motion.span>
      ))}
...
function WhyBox({ label, value, direction, delay }) {
  ...
  return (
    ...
    >
      <p style={{ fontSize: '9px', fontWeight: '600', color: C.amber, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '6px' }}>
        {label}
      </p>
      <p style={{ fontSize: '12px', color: C.textMuted, lineHeight: '1.6' }}>{value}</p>
    </motion.div>
  )
}
  ...
  return (
    <div ref={ref} style={{ position: 'relative', paddingLeft: '28px' }}>
      <div style={{
        ...
        background: C.border, overflow: 'hidden', bottom: '6px',
      }}>
        <motion.div
          initial={{ height: 0 }}
          animate={drawn ? { height: '100%' } : {}}
          transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] }}
          style={{ width: '100%', background: `linear-gradient(to bottom, ${C.amber}, ${C.amber}22)`, boxShadow: `0 0 4px ${C.amber}` }}
        />
      </div>
      ...
      {plan.map((item, i) => (
        <motion.div
          ...
          transition={{ delay: 0.3 + i * 0.12, type: 'spring', stiffness: 260, damping: 22 }}
          style={{ display: 'flex', gap: '14px', marginBottom: '16px', position: 'relative', alignItems: 'flex-start' }}
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={drawn ? { scale: 1 } : {}}
            transition={{ delay: 0.4 + i * 0.12, type: 'spring', stiffness: 400, damping: 20 }}
            style={{
              position: 'absolute', left: '-24px', top: '8px',
              width: '10px', height: '10px', borderRadius: '50%',
              background: i === 0 ? C.amber : 'rgba(30,42,58,0.8)',
              border: `1px solid ${i === 0 ? C.amber : C.border}`,
              boxShadow: i === 0 ? `0 0 8px ${C.amber}` : 'none', zIndex: 1,
            }}
          />
          <motion.span
            initial={{ scale: 1.4, rotate: -3 }}
            animate={drawn ? { scale: 1, rotate: 0 } : {}}
            transition={{ delay: 0.5 + i * 0.12, type: 'spring', stiffness: 500, damping: 25 }}
            style={{
              fontSize: '9px', fontWeight: '700', color: C.amber,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: '13px', color: C.text, lineHeight: '1.5', marginBottom: item.goal ? '3px' : 0 }}>
              {item.task}
            </p>
            {item.goal && (
              <motion.p
...
function ResourceItem({ resource, index }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const interval = setInterval(() => {
        if (i < url.length) { setTypedUrl(url.slice(0, i + 1)); i++ }
  ...
  return (
    <motion.div
      ...
      style={{ borderLeft: `2px solid ${C.tealBorder}`, paddingLeft: '12px', marginBottom: '12px' }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px', flexWrap: 'wrap' }}>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          style={{ fontSize: '13px', color: C.teal, textDecoration: 'none', fontWeight: '500' }}
        >
          {resource.title}
        </a>
        {resource.type && (
          <span style={{
            fontSize: '9px', color: C.textMuted, background: 'rgba(30,42,58,0.8)',
      ...
      {showUrl && (
        <p style={{ fontSize: '10px', color: C.textDim, fontFamily: 'monospace', marginBottom: '2px', wordBreak: 'break-all' }}>
          {typedUrl}
          {typedUrl.length < (resource.url?.length || 0) && <span style={{ color: C.teal }}>|</span>}
        </p>
      )}
      {resource.use_on && (
        <p style={{ fontSize: '10px', color: C.textMuted }}>Use on: {resource.use_on}</p>
      )}
      {resource.why_this_one && (
        <p style={{ fontSize: '11px', color: C.textMuted, lineHeight: '1.5' }}>{resource.why_this_one}</p>
      )}
    </motion.div>
  )
}
        ...
        >
          ...
          {[
            ...
            <motion.div
              ...
              style={{ position: 'absolute', width: '20px', height: '20px', ...style }}
            />
          ))}
          ...
          {/* why grid — 3 cols desktop, 1 col mobile */}
          <div style={{
            display: 'grid',
          ...
          <div style={{
            ...
            {nextStep.what_you_will_have_after && (
              ...
              >
                <p style={{ fontSize: '9px', color: C.teal, fontWeight: '600', marginBottom: '6px', letterSpacing: '0.1em' }}>YOU WILL HAVE</p>
            ...
            {nextStep.time_estimate && (
              ...
              >
                <p style={{ fontSize: '9px', color: C.amber, fontWeight: '600', marginBottom: '6px', letterSpacing: '0.1em' }}>TIME NEEDED</p>
                <div style={{ display: 'flex', gap: '6px' }}>
          ...
          {nextStep.week_plan?.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <p style={{ fontSize: '9px', fontWeight: '600', color: C.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '16px' }}>
                Day by day plan
              </p>
          ...
          {nextStep.resources?.length > 0 && (
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: '20px' }}>
              <p style={{ fontSize: '9px', fontWeight: '600', color: C.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '14px' }}>
